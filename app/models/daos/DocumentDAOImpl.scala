...
class DocumentDAOImpl @Inject() (val dbConfigProvider: DatabaseConfigProvider)(implicit ec: ExecutionContext)
  ...
  def search(query: String, page: Int, pageSize: Int): Future[Page[SearchResult]] = {
    ...
    val searchResultsQuery = totalQuery.map(t => (t.id, tsHeadline(t.content, webSearchToTsQuery(query)),
        tsRankCD(t.searchField, webSearchToTsQuery(query)))).sortBy(_._3.desc)
    ...
    for {
      searchResults <- db.run(paginatedQuery.result) map {
        results => results.map {
          case (id, highlight, rank) => SearchResult(highlights = highlight, rank = rank, documentId = id)
        }
      }
